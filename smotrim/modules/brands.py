...
from smotrim.modules import pages, persons
...
class Brand(pages.Page):
    ...
    def search(self):
        if "search" not in self.params:
            ...
            if self.search_text:
                from smotrim.modules import searches
    ...
    def get_nav_url(self, offset=0):
        if self.action == "search" and self.search_text:
            return get_url(self.site.url, action=self.action,
                           ...
                           limit=self.limit,
                           url=self.site.url)
        if self.action == "search_by_tag" and self.search_tag:
            return get_url(self.site.url, action=self.action,
                           context="brands",
                           ...
                           limit=self.limit,
                           url=self.site.url)
        if self.action == "search_by_person" and self.search_person:
            return get_url(self.site.url, action=self.action,
                           context="brands",
